...
const RightBar = () => {
  ...
  const { socket, peer, setPlayerKeys, setPeerConn, isAdmin } =
    ...
    <>
      ...
      <div className="fixed bottom-3 right-2 z-20 flex">
        ...
        >
          <div className="mt-3 ml-[0.1rem]">
            ...
            <div
              className={`bg-[#ffff3d] w-3 h-3 relative -top-8 left-5 rounded-full ${
                chatDot ? "" : "hidden"
              }`}
            ></div>
          </div>
        ...
        <button
          onClick={onLeave}
          className="bg-gray-300 px-2 h-12 rounded-[100px] flex justify-center text-center hover:bg-white ml-2"
        >
      ...
      >
        <button
          ...
          onClick={() => setBoxes({ ...boxes, leave: false })}
        >
          <IoClose size={25} color="#fff" />
        </button>
        <div className="py-3 px-8 flex flex-col mt-2">
          <button
            className="bg-[#db3954] rounded hover:scale-110 duration-300"
            onClick={disconnectAlone}
          >
            Leave Meeting
          </button>
          <button
            className={`bg-[#db3954] mt-4 rounded px-2 ${
              isAdmin ? "hover:scale-110 duration-300" : "opacity-60"
            }`}
            disabled={!isAdmin}
            onClick={disconnectEveryone}
          >
            End Meeting for All
          </button>
        </div>
