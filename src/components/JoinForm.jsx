    ...
    connectSocket(lowercaseMeetingId).then((value) => {
      if (value && value.room) {
        ...
        setError({
          ...error,
          meetingId: true,
          meetingMessage: "Meeting ID does not exist",
        });
      }
    });
  };
      ...
      <div className="flex items-center justify-center h-screen">
        ...
        <div className="p-2 bg-gradient-to-r from-gray-500 via-slate-500 to-gray-500 rounded border-2">
          ...
          <form onSubmit={handleSubmit}>
            <div>
              <label htmlFor="meetingId" className="text-lg">
                Meeting ID:{" "}
              </label>
            </div>
            <div>
            ...
            {error.meetingId && (
              <p className="text-[red] text-sm mt-1">{error.meetingMessage}</p>
            )}
          ...
          <div className="mt-2 text-center">
            ...
            <button
              onClick={onNewMeet}
              className="mt-2 border bg-[#5c89d1] p-1 rounded hover:scale-105 duration-300"
            >
              Create Meeting
            </button>
          </div>
